...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##
## Subsection: DCM Categorical Vertical structure w/ False-positives
##
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#   
#   ## The sheer size of the graph being made in the compilation phase
#   ## might force me to do validation on either a sample of sites and years
#   ## or perform validation externally via PPC from the monitored parameters
#   
#   ## Open part of the model
#   ## Get initial simulation of zrep
#   for(i in 1:nsites){
#     for(t in 1:nyears){
#       for(k in 1:nspec){
...
# 
#   for(k in 1:nspec){
#     for(t in 1:nyearsminus1){
#       ## Loop the sites
#       for(i in 1:nsites){
#         ## Transitions states
#         ## Absent -> Absent
#         n00_obs[i,t,k] <- equals(z[i,t,k],0) * equals(z[i,t+1,k],0) * (sampled[i,t] * sampled[i,t+1])
#         ## Colonization
#         n01_obs[i,t,k] <- equals(z[i,t,k],0) * equals(z[i,t+1,k],1) * (sampled[i,t] * sampled[i,t+1])
#         ## Extinction
#         n10_obs[i,t,k] <- equals(z[i,t,k],1) * equals(z[i,t+1,k],0) * (sampled[i,t] * sampled[i,t+1])
#         ## Present -> Present
#         n11_obs[i,t,k] <- equals(z[i,t,k],1) * equals(z[i,t+1,k],1) * (sampled[i,t] * sampled[i,t+1])
# 
#         ## Count from replicates
#         n00_rep[i,t,k] <- equals(z_rep[i,t,k],0) * equals(z_rep[i,t+1,k],0) * (sampled[i,t] * sampled[i,t+1])
#         ## Colonization
#         n01_rep[i,t,k] <- equals(z_rep[i,t,k],0) * equals(z_rep[i,t+1,k],1) * (sampled[i,t] * sampled[i,t+1])
#         ## Extinction
#         n10_rep[i,t,k] <- equals(z_rep[i,t,k],1) * equals(z_rep[i,t+1,k],0) * (sampled[i,t] * sampled[i,t+1])
#         ## Present -> Present
#         n11_rep[i,t,k] <- equals(z_rep[i,t,k],1) * equals(z_rep[i,t+1,k],1) * (sampled[i,t] * sampled[i,t+1])
#       }
# 
#       ## Summation over sites
#       n00_obs_sum[t,k] <- sum(n00_obs[1:nsites,t,k])
#       n01_obs_sum[t,k] <- sum(n01_obs[1:nsites,t,k])
#       n10_obs_sum[t,k] <- sum(n10_obs[1:nsites,t,k])
#       n11_obs_sum[t,k] <- sum(n11_obs[1:nsites,t,k])
# 
#       ## Replicates
#       n00_rep_sum[t,k] <- sum(n00_rep[1:nsites,t,k])
#       n01_rep_sum[t,k] <- sum(n01_rep[1:nsites,t,k])
#       n10_rep_sum[t,k] <- sum(n10_rep[1:nsites,t,k])
#       n11_rep_sum[t,k] <- sum(n11_rep[1:nsites,t,k])
# 
# 
# 
#       ## Expectation for the counts based on model params
#       ## Add indicator to deal with unsurveyed transitions
#       exp00[t,k] <- sum((1-psi[1:nsites,t,k]) * (1-gamma[1:nsites,t,k]) *
#                           (sampled[1:nsites,t] * sampled[1:nsites,t+1]))
...
# 
#       ## Discrepancy metrics
#       chisq_open_obs[t,k] <- pow(n00_obs_sum[t,k] - exp00[t,k], 2) / (exp00[t,k] + 0.001) +
#         pow(n01_obs_sum[t,k] - exp01[t,k], 2) / (exp01[t,k] + 0.001) +
#         pow(n10_obs_sum[t,k] - exp10[t,k], 2) / (exp10[t,k] + 0.001) +
#         pow(n11_obs_sum[t,k] - exp11[t,k], 2) / (exp11[t,k] + 0.001)
# 
#       chisq_open_rep[t,k] <- pow(n00_rep_sum[t,k] - exp00[t,k], 2) / (exp00[t,k] + 0.001) +
#         pow(n01_rep_sum[t,k] - exp01[t,k], 2) / (exp01[t,k] + 0.001) +
#         pow(n10_rep_sum[t,k] - exp10[t,k], 2) / (exp10[t,k] + 0.001) +
#         pow(n11_rep_sum[t,k] - exp11[t,k], 2) / (exp11[t,k] + 0.001)
#     }
#   }
# 
#   ## Take the sum
#   chi_open_obs <- sum(chisq_open_obs[1:nyearsminus1, 1:nspec])
#   chi_open_rep <- sum(chisq_open_rep[1:nyearsminus1, 1:nspec])
#   ## Use step to comparise
#   fit_open <- step(chi_open_rep - chi_open_obs)
# 
#   ## Species-specific chi-sq
#   for(k in 1:nspec){
#     ## Take the sum
#     chi_open_obs_sp[k] <- sum(chisq_open_obs[1:nyearsminus1, k])
#     chi_open_rep_sp[k] <- sum(chisq_open_rep[1:nyearsminus1, k])
#     ## Use step to comparise
#     fit_open_sp[k] <- step(chi_open_rep_sp[k] - chi_open_obs_sp[k])
#   }
# 
...
#   ## Section Notes:
#   ## To-Do
#   ## 1. Reduce calculations from loops if possible
#   ## 2. Remove unobserved site x year combinations
#   ##
#   ## ***********************************************************
#   # for(i in 1:nsites) {
#   #   for(t in 1:nyears) {
#   #     for(k in 1:nspec) {
#   #       for(r in 1:max_reps) {
#   #         y_det[i,t,r,k] <- is_valid_visit[i,t,r] * y[visit_ind[i,t,r],k]
#   #         p_mix[visit_ind[i,t,r],k] <- is_valid_visit[i,t,r] *
#   #           (z[i,t,k] * p_obs[visit_ind[i,t,r],k] + (1 - z[i,t,k]) * ilogit(alphaFP[k]))
#   #         p_mix_det[i,t,r,k] <- p_mix[visit_ind[i,t,r],k] * is_valid_visit[i,t,r]
#   #         yrep_det[i,t,r,k] ~ dbern(p_mix[visit_ind[i,t,r],k] * is_valid_visit[i,t,r])
#   #       }
#   #       
#   #       ## sum over max_reps, mask makes unused visits = 0
#   #       y_sum[i,t,k]     <- sum(y_det[i,t, 1:max_reps, k])
#   #       yrep_sum[i,t,k]  <- sum(yrep_det[i,t, 1:max_reps, k])
#   #       exp_closed[i,t,k] <- sum(p_mix_det[i,t, 1:max_reps, k])
#   #       
...
#   # 
#   # ## Sum globally
#   # ## We need to split into two stages for nimble
#   # for(i in 1:nsites){
#   #   for(t in 1:nyears){
#   #   chi_closed_obs_int[i,t] <- sum(chisq_closed_obs[i, t, 1:nspec])
#   #   chi_closed_rep_int[i,t] <- sum(chisq_closed_rep[i, t, 1:nspec])
#   # 
...
#   #   }
#   # }
#   # chi_closed_obs <- sum(chi_closed_obs_int[1:nsites, 1:nyears])
#   # chi_closed_rep <- sum(chi_closed_rep_int[1:nsites, 1:nyears])
#   # fit_closed <- step(chi_closed_rep - chi_closed_obs)
#   
#   ##----------------------
#   ## Derived parameters
...
## Section Notes:
## To-Do:
## 1. Shorten the syntax to use loops for priors
## 2. Shorten the syntax to use inprod for flexibility
## ***********************************************************
...
bdata <- readRDS(here("Data/Occ_Data/DCM_Ragged_Full_2021_2025_FireCont.RDS"))  
...
#   "alphaFP",
#   "fp_rate",
#   ## Latent state (for PPCs)
#   "z",
#   ## Trait
#   paste0(rep(paste0("delta", 0:2, ".gamma"), each = 12), 0:11),
#   paste0(rep(paste0("delta", 0:2, ".eps"), each = 12), 0:11)
# )
# params
...
## Write to file
saveRDS(samples, file = "D:/DCM_Samples/DCMmodel_mcmc_output_FP_21_25_Traits.rds")
